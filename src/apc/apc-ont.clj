...
(def locations
  (doall (map #(individual-with-type % Location) location)))
...
(def current-status
  (doall (map #(individual-with-type % Status) status)))
;; create list of species as strings
(def species-name
  (doall (map #(individual-with-type % Species) species)))
;; create strings list of cell types
(def cell-types
  (doall (map #(individual-with-type % CellType) cell-type)))
;; create strings list of description
(def descriptions
  (doall (map #(individual-with-type % Description) description)))
;; create strings list of activation process
(def activ
  (doall (map #(individual-with-type % Activation) activation)))
;; create strings list of antigen loading
(def antigen-load
  (doall (map #(individual-with-type % AntigenLoad) antigen-loading)))
;; create strings list of cell origins in the sheet
(def c-origin
  (doall (map #(individual-with-type % CellOrigin) cell-origin)))
;; create strings list of starting materials
(def start-materials
  (doall (map #(individual-with-type % StartMaterial) starting-material)))
;; create strings list of isolation method
(def isolations
  (doall (map #(individual-with-type % Isolation) isolation)))

;; create cell line with all info
(defn cell-line
  [cell-name group loc clinic-disease stat from-species
   c-type desc active anti-load cell-org start-material isol]
